...
- Ручной режим и режим TrustTunnel DeepLink для настройки туннеля
...
- Опциональный сплит-туннелинг на базе GeoIP (страновые/ручные политики bypass или tunnel через ipset + policy routing)
...
trusttunnel-keenetic (Rust wrapper)
  ...
  |
  +- GeoIP сервис (компактная бинарная база + опциональные API-лукапы)
  +- Split Tunnel Manager (компилятор политик в ipset/iptables/policy routing)
  \- Планировщик автообновления GeoIP (фоновый поток)
```
...
- DeepLink содержит данные endpoint в кодированном, но не зашифрованном виде; перед сохранением проверьте импортированные значения.
