...
- `disconnect` останавливает процесс туннеля, но не завершает поток супервизора; следующий `connect` остаётся под контролем мониторинга и автоподнятия.
- Wrapper постоянно вычитывает `stdout`/`stderr` дочернего клиента, чтобы исключить зависание из-за переполнения pipe-буферов при подробных логах.
- Если routing ещё не активирован, watchdog инициирует повторную настройку routing вместо «тихого» отключения проверок.
- Для проверки связности watchdog требуется `curl` с поддержкой `--interface`, чтобы проверять именно канал через `opkgtun0`.
